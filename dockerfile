...
RUN uv pip install --system -e . \
    ...
    && find /usr/local/lib/python3.*/dist-packages/whisperx -name "diarize.py" -type f -exec python3 -c "import re, sys; f=sys.argv[1]; c=open(f).read(); o=c; c=re.sub(r'pd\.DataFrame\((\w+)\.itertracks\(yield_label=True\)', r'pd.DataFrame([(segment, label, label) for segment, track, label in (\1.annotation if hasattr(\1, \"annotation\") else \1).itertracks(yield_label=True)]', c); open(f,'w').write(c) if c!=o else None; print(f'Patched: {f}') if c!=o else None" {} \; 2>/dev/null || true \
    && find /usr/local/lib/python3.*/dist-packages/whisperx -name "diarize.py" -type f -exec python3 -c "import re; import glob; [exec('with open(f,\"r\") as r: c=r.read(); o=c; c=re.sub(r\"diarization\.itertracks\(yield_label=True\)\", \"[(segment, label, label) for segment, track, label in diarization.itertracks(yield_label=True)]\", c); (open(f,\"w\").write(c) if c!=o else None)') for f in glob.glob('/usr/local/lib/python3.*/dist-packages/whisperx/diarize.py')]" 2>/dev/null || true \
