...
RUN uv pip install --system -e . \
    ...
    && python3 -c "import gunicorn; import uvicorn; print(f'✓ gunicorn={gunicorn.__version__}, uvicorn={uvicorn.__version__}')" || (echo "ERROR: gunicorn or uvicorn not found!" && python3 -c "import sys; print('Python paths:', sys.path)" && exit 1) \
    ...
    && echo "Installing soundfile for audio backend support..." \
    && uv pip install --system --no-cache-dir "soundfile>=0.12.0" \
